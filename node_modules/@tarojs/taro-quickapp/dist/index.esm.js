...
import device$1 from '@system.device';
...
function updateComponent(component) {
  ...
  if (component.__mounted) {
    if (isFunction(component.shouldComponentUpdate) && component.shouldComponentUpdate(props, state) === false) {
      skip = true;
...
function doUpdate(component, prevProps, prevState) {
  ...
  if (component.$usedState && component.$usedState.length) {
    ...
    component.$usedState.forEach(function (key) {
      var val = internal_safe_get(data, key);
      ...
      if (_typeof(val) === 'object') {
        if (isEmptyObject(val)) return internal_safe_set(_data, key, val);
        val = shakeFnFromObject(val); // 避免筛选完 Fn 后产生了空对象还去渲染

        if (!isEmptyObject(val)) internal_safe_set(_data, key, val);
      } else {
        internal_safe_set(_data, key, val);
      }
    });
    data = _data;
  }
...
function () {
  ...
  _createClass(BaseComponent, [{
    ...
    value: function setState(state, callback) {
      ...
      if (!this._disable) {
        enqueueRender(this, internal_force_update === callback);
      }
    }
  }, {
    key: "getState",
    ...
    value: function __triggerPropsFn(key, args) {
      ...
      if (reduxFnName in this) {
        ...
        if (keyChain.length > 0) {
          fn = internal_safe_get(this[reduxFnName], keyChain.join('.'));
        } else {
          fn = this[reduxFnName];
        }
...
function qappNavigate() {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var parseUrl = addLeadingSlash$1(url.substr(0, markIndex >= 0 ? markIndex : url.length));
...
var Taro = {
  ...
  render: render,
  ENV_TYPE: ENV_TYPE,
  internal_safe_get: internal_safe_get,
  internal_safe_set: internal_safe_set,
  internal_inline_style: internal_inline_style,
  createComponent: createComponent,
  internal_get_original: internal_get_original,
